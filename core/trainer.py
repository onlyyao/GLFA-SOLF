...
class Trainer(object):
    ...
    def _train(self, epoch_idx):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            # output, acc, loss, loss_cr, loss_BT = self.model.set_forward_loss(batch)
            output, acc, loss, gamma_loss, alpha_loss = self.model.set_forward_loss(batch)
            # output, acc, loss, gamma_loss, alpha_loss, loss_BT = self.model.set_forward_loss(batch)
            ...
            # meter.update("loss_cr", loss_cr.item())
            meter.update("gamma_loss", gamma_loss.item())
            meter.update("alpha_loss", alpha_loss.item())
            # meter.update("loss_BT", loss_BT.item())
            ...
            if (batch_idx != 0 and (batch_idx + 1) % self.config["log_interval"] == 0) or (
                ...
                info_str = (
                    ...
                    "Acc@1 {:.3f} ({:.3f})".format(
                        ...
                        # meter.last("loss_cr"),
                        # meter.avg("loss_cr"),
                        meter.last("gamma_loss"),
                        meter.avg("gamma_loss"),
                        meter.last("alpha_loss"),
                        meter.avg("alpha_loss"),
                        # meter.last("loss_BT"),
                        # meter.avg("loss_BT"),
                        meter.last("acc1"),
                        meter.avg("acc1"),
    ...
    def _init_meter(self):
        ...
        train_meter = AverageMeter(
            "train",
            ["batch_time", "data_time", "calc_time", "loss", "loss_cr", "loss_BT", "gamma_loss", "alpha_loss","acc1"],
            self.writer,
        )
